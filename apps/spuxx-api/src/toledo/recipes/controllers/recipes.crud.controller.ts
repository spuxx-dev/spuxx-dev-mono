...
import {
  RecipesFindByIdQuery,
  RecipesFindManyQuery,
  RecipesUpdateQuery,
} from './queries';
...
export class RecipesCrudController {
  ...
  async findMany(
    ...
    const recipes = await this.provider.findMany(
      request,
      transformQueryToFindOptions(query)
    );
    return recipes.map((recipe) =>
      this.mapper.map(recipe, Recipe, RecipeReadResource)
    );
  }
  ...
  async findById(
    ...
    const recipe = await this.provider.findById(
      id,
      request,
      transformQueryToFindOptions(query)
    );
